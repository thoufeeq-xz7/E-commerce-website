...
def index():
    ...
    if category:
        items = [
            it for it in items if category == it.get("category", "").lower()
        ]
...
def edit_item(item_id: str):
    ...
    # Pre-fill sizes input for template
...
def update_cart_item():
    ...
    # Merge with existing item of same id + size
    for item in cart:
...
def recent_orders():
    ...
    def _parsed_created(o: dict):
        ...
        try:
            # created_at stored like ISO with trailing Z (UTC)
            if val.endswith("Z"):
                val = val[:-1]
